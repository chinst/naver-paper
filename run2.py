...
def init(id, pwd, ua, mobile_device, headless, newsave):
    ...
    chrome_options.add_argument(f"--remote-allow-origins=*")
    ...
    if driver.title == "Naver ID" or driver.title == "네이버ID":
        print(f'init: title = {driver.title}')
        return driver
...
if __name__ == "__main__":
    ...
    if True: #(args.id is None and args.pw is None and args.cd is None and args.credential_file is None):
        id = 'chinst' #os.getenv("USERNAME")
        pw = 'ckaRo$69' #os.getenv("PASSWORD")
